export default {
  menuList: [
    {
      text: 'Asosiy',
      path: 'main',
      name: 'main',
      imglink: 'https://img.icons8.com/glyph-neue/64/home--v1.png'
    },
    {
      text: 'Tarixiy obidalar',
      path: 'destinations',
      name: 'destinations',
      imglink: 'https://img.icons8.com/ios/50/reichstag.png'
    },
    // {
    //   text: 'Turar joy',
    //   path: 'accommodation',
    //   name: 'accommodation',
    //   imglink:
    //     'https://img.icons8.com/external-flaticons-lineal-color-flat-icons/64/000000/external-accomodation-hotel-management-flaticons-lineal-color-flat-icons.png'
    // },
    // {
    //   text: 'Transport',
    //   path: 'transportation',
    //   name: 'transportation',
    //   imglink: 'https://img.icons8.com/glyph-neue/64/ground-transportation.png'
    // },
    // {
    //   text: 'Viza va sayohat',
    //   path: 'visa',
    //   name: 'visa',
    //   imglink: 'https://img.icons8.com/ios-filled/50/visa.png'
    // },
    // {
    //   text: 'Oshxona va ovqatlanish',
    //   path: 'cuisine',
    //   name: 'cuisine',
    //   imglink: 'https://img.icons8.com/glyph-neue/64/restaurant-membership-card.png'
    // },
    {
      text: 'Ob-havo va iqlim',
      path: 'weather',
      name: 'weather',
      imglink: 'https://img.icons8.com/ios-filled/50/climate-change.png'
    },
    {
      text: 'Valyuta konvertori',
      path: 'converter',
      name: 'converter',
      imglink: 'https://img.icons8.com/dotty/80/currency-exchange.png'
    }
  ],
  currenciesData: [
    'AED',
    'ARS',
    'AUD',
    'BGN',
    'BRL',
    'BSD',
    'CAD',
    'CHF',
    'CLP',
    'CNY',
    'COP',
    'CZK',
    'DKK',
    'DOP',
    'EGP',
    'EUR',
    'FJD',
    'GBP',
    'GTQ',
    'HKD',
    'HRK',
    'HUF',
    'IDR',
    'ILS',
    'INR',
    'ISK',
    'JPY',
    'KRW',
    'KZT',
    'MXN',
    'MYR',
    'NOK',
    'NZD',
    'PAB',
    'PEN',
    'PHP',
    'PKR',
    'PLN',
    'PYG',
    'RON',
    'RUB',
    'SAR',
    'SEK',
    'SGD',
    'THB',
    'TRY',
    'TWD',
    'UAH',
    'USD',
    'UZS',
    'UYU',
    'VND',
    'ZAR'
  ],
  headerCardInfo: [
    {
      title: 'Tabiat',
      subtitle:
        "Go'zal tog'lar, gullab-yashnagan vodiylar, issiq cho'llar, daryolar va billur ko'llar - bularning barchasini O'zbekistonda bahramand bo'lishingiz mumkin!",
      backImg:
        'https://uzbekistan.travel/storage/app/uploads/public/604/9ea/e3d/thumb_1608_1920_0_0_0_auto.jpg',
      btnText: "Ko'proq"
    },
    {
      title: "O'zbek oshxonasi",
      subtitle:
        "Xushbo'y palov, mazali qozon-kabob, samsa, non va issiq sho'rpa - o'zining rang-barangligi va beqiyos ta'mi bilan O'zbekiston aholisi va mehmonlarini lol qoldiradi.",
      backImg:
        'https://uzbekistan.travel/storage/app/uploads/public/5fb/e45/3aa/thumb_1425_1920_0_0_0_auto.jpg',
      btnText: "Ko'proq"
    },
    {
      title: 'Arxitektura',
      subtitle:
        'Moviy mozaika va rang-barang rasmlar bilan bezatilgan muhtasham meʼmoriy ansambllar, saroylar, masjidlar va minoralar koʻp asrlardan buyon Oʻzbekistonning millionlab mehmonlari koʻzini hayratga solib kelmoqda.',
      backImg:
        'https://uzbekistan.travel/storage/app/uploads/public/604/9ea/9bc/thumb_1607_1920_0_0_0_auto.jpg',
      btnText: "Ko'proq"
    },
    {
      title: "O'zbekistonning madaniy merosi",
      subtitle:
        'Oʻzbekiston madaniyati Markaziy Osiyo xalqlarining koʻp asrlik anʼanalari va turmush tarzi bilan uzviy bogʻliq boʻlgan boy tarixga ega.',
      backImg:
        'https://uzbekistan.travel/storage/app/uploads/public/5fb/e45/83a/thumb_1427_1920_0_0_0_auto.jpg',
      btnText: "Ko'proq"
    },
    {
      title: 'Xalq ijodiyoti',
      subtitle:
        'Chuqur ma’no va ahamiyati ko‘p asrlar davomida shakllangan O‘zbekiston amaliy san’ati mahsulotlarini to‘qadigan noyob naqshlar',
      backImg:
        'https://uzbekistan.travel/storage/app/uploads/public/5fb/f31/004/thumb_1430_1920_0_0_0_auto.jpg',
      btnText: "Ko'proq"
    }
  ],
  regionCard: [
    {
      regionName: 'Toshkent',
      regionAdvantage: 'Zamonaviy megapolis',
      regionImg:
        'https://uzbekistan.travel/storage/app/uploads/public/5eb/46f/6e1/thumb_764_600_0_0_0_auto.jpg'
    },
    {
      regionName: 'Samarqand',
      regionAdvantage: 'Madaniyatlar chorrahasi',
      regionImg:
        'https://uzbekistan.travel/storage/app/uploads/public/5eb/46f/ce0/thumb_765_600_0_0_0_auto.jpg'
    },
    {
      regionName: 'Buxoro',
      regionAdvantage: 'Islom madaniyatining poytaxti',
      regionImg:
        'https://uzbekistan.travel/storage/app/uploads/public/5eb/46f/e7c/thumb_766_600_0_0_0_auto.jpg'
    },
    {
      regionName: 'Xiva',
      regionAdvantage: 'Turk dunyosining poytaxti',
      regionImg:
        'https://uzbekistan.travel/storage/app/uploads/public/5eb/470/0be/thumb_767_600_0_0_0_auto.jpg'
    },
    {
      regionName: 'Shahrisabz',
      regionAdvantage: 'Amir Temurning vatani',
      regionImg:
        'https://uzbekistan.travel/storage/app/uploads/public/5eb/470/5d1/thumb_769_600_0_0_0_auto.jpg'
    },
    {
      regionName: 'Mo‘ynoq',
      regionAdvantage: 'Orol sahrosi',
      regionImg:
        'https://uzbekistan.travel/storage/app/uploads/public/5eb/473/e47/thumb_771_600_0_0_0_auto.jpg'
    },
    {
      regionName: 'Zomin',
      regionAdvantage: 'O‘zbek Shveytsariyasi',
      regionImg:
        'https://uzbekistan.travel/storage/app/uploads/public/5eb/476/f7b/thumb_774_600_0_0_0_auto.jpg'
    },
    {
      regionName: 'Termiz',
      regionAdvantage: 'Qadimiy tsivilizatsiya va din markazi',
      regionImg:
        "https://uzbekistan.travel/storage/app/uploads/public/5eb/478/9a5/thumb_776_600_0_0_0_auto.jpg"
    },
  ]
}
